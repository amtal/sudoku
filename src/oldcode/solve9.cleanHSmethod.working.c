...
inline int detectHiddenSingles()
{
	...
	for (i=0; i<WIDTH; i++) {
		...
		for (j=0; j<WIDTH; j++) {
			int pos = i*WIDTH+j;
			bitboard row = getFastMask(pos); //pgrid[pos];
		...
		while (repeater) {
			...
			// find exact position of hidden value
			for (j=0; j<WIDTH; j++) {
				pos = i*WIDTH+j; // exit loop when non-zero value found
				if ((hidden = repeater&getFastMask(pos))) {break;}
			}
			//logit("%x row c%dr%d\n", hidden, j, i);
			// assign
		...
		for (j=0; j<WIDTH; j++) {
			int pos = i+j*WIDTH;
			bitboard col = getFastMask(pos); // pgrid[pos];
			repeater |= (accumulator & col) | grid[pos];
			accumulator |= col;
			//printf("acc:%x rep:%x col: %x j:%d\n",
			//		accumulator, repeater, col, j);
		}
		repeater = ~repeater & MAXMASK;
		...
		while (repeater) {
			...
			// find exact position of hidden value
			for (j=0; j<WIDTH; j++) {
				pos = i+j*WIDTH; // exit loop when non-zero value found
				if ((hidden = repeater&getFastMask(pos))) {break;}
			}
			//logit("%x col c%dr%d\n", hidden, i, j);
			// assign
		...
		//
		// ITERATING OVER BLOCKS (blocks or boxes? hmmm...)
		// 
		accumulator=repeater=0;
		for (j=0; j<WIDTH; j++) { 
			// i the the box number, j is the position in the box
			// Lookup tables, is there anything they can't do?!
			// Hm. Alternatively, could use:
			//  	box term: i%WIDTH/BLOCK*BLOCK + 
			//  +(i-i%width)/(BLOCK*WIDTH)(BLOCK*WIDTH)
			//  	position term: eh why even bother 8 operations 
			//  allready the blockAddrLookup table sped up a much 
			// simpler operation.
			//
			// Alternatively, could use i*j in an 81 value lookup
			// table. That should be faster, but... Noticeably so? 
			// Probably not.
			int pos = blockShortAddrLookup[i]+blockShortOffsetLookup[j];
			bitboard row = getFastMask(pos); //pgrid[pos];
			repeater |= (accumulator & row) | grid[pos];
			accumulator |= row;
			//printf("acc:%x rep:%x row: %x j:%d\n",
		...
		while (repeater) {
			...
			// find exact position of hidden value
			for (j=0; j<WIDTH; j++) {
				pos = blockShortAddrLookup[i]+blockShortOffsetLookup[j]; // exit loop when non-zero value found
				if ((hidden = repeater&getFastMask(pos))) {break;}
			}
			logit("%x block b%dp%d\n", hidden, i, j);
			// assign
			grid[pos] = hidden;	
			impossible(pos, hidden);
			found++;
